...
const labels={'not-built':'No dedicated lesson',partial:'Partial teaching',intro:'Introduction only'};
let audit;
function render(){
 const term=$('search').value.trim().toLowerCase(), domain=$('domain').value, status=$('status').value;
 const rows=audit.rows.filter(r=>(domain==='all'||r.section===domain)&&(status==='all'||(status==='no-questions'?!r.questionIds.length:r.evidence.status===status))&&(!term||[r.id,r.title,r.lesson,r.evidence.text,...r.topics.map(t=>t.title)].join(' ').toLowerCase().includes(term)));
 ...
 for(const r of rows){
  ...
  body.append(el('p','Planned teaching: '+r.lesson));
  const list=el('ul',undefined,'topics');for(const topic of r.topics){const li=el('li',topic.title);li.append(el('small',topic.status==='draft teaching'?'Draft teaching exists · review pending':'Teaching still planned'));list.append(li);}body.append(list);
  body.append(el('p','Scenario to build: '+r.example));body.append(el('p','Practice task: '+r.assessment));
  body.append(el('p',r.questionIds.length?'Primary bank references: '+r.questionIds.map(id=>'Q'+id+(audit.questionAudit[id].status==='held'?' (held)':'')).join(', ')+'. These do not cover every checkpoint.':'No primary questions in the original bank. New assessment items required.'));
...
async function init(){try{
 ...
 $('metrics').replaceChildren();for(const [value,label]of [[audit.summary.subsections,'outline subsections mapped'],[audit.summary.topicGroups,'AMG teaching checkpoints'],[audit.summary.revised,'questions revised'],[audit.summary.held,'questions held for review']]){const box=el('div',undefined,'metric');box.append(el('strong',String(value)),el('span',label));$('metrics').append(box);}
