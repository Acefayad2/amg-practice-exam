(async () => {
 ...
 function dashboard(){
  ...
  const next=progress.find(x=>!x.complete);panel.append(node('p',next?'Pick up where you left off.':'All lesson requirements are complete.'),link(next?'Continue with Part '+next.lesson.number:'Review your course',next?'/course/lesson-'+next.lesson.id+'/':'/course/'));
 ...
 function renderEvidence(complete){
  ...
  section.append(node('p',met?'The course and two-form study target are met. Review the domain evidence below before deciding your next step.':complete+' / 60 parts · '+hits.length+' fresh forms at 85%+ · '+days.size+' qualifying study days · '+outstanding+' mock explanations awaiting review.','notice'));
