...
const credentialPath=option('--credentials')||'/tmp/amg-account-live-qa.json';
const outputPath=option('--output')||'output/qa/account-live-login.json';
...
async function main(){
 ...
 try {
  ...
  for(const label of ['A','B']){const a=config.accounts?.[label];assert.ok(a&&typeof a.id==='string'&&a.id&&typeof a.email==='string'&&a.email.includes('@')&&typeof a.password==='string'&&a.password.length>=12,'Both pre-created synthetic accounts are required.');}
  assert.notEqual(config.accounts.A.id,config.accounts.B.id);assert.notEqual(config.accounts.A.email,config.accounts.B.email);
  ...
  const accountRecord=async(page,expected)=>{
   ...
   check(body.user?.id===expected.id&&body.user?.email?.toLowerCase()===expected.email.toLowerCase(),stage+': server identity matches the intended synthetic account');
  ...
  const login=async(page,account,next='/course/')=>{
   ...
   check(true,stage+': password login succeeds through the real login form');
  };
  const signOut=async page=>{
   await page.getByRole('button',{name:'Sign out',exact:true}).click();
   await page.waitForURL(url=>url.origin===origin&&url.pathname==='/login/',{timeout:30000});
   await page.locator('#login-form').waitFor({state:'visible'});
   check(await page.locator('#login-password').inputValue()==='',stage+': signout returns to an empty masked password field');
   const response=await page.request.get(origin+'/api/learning',{failOnStatusCode:false});
  ...
  stage='account-A-login';await login(page,config.accounts.A);
  const first=await accountRecord(page,config.accounts.A);
  ...
  await page.waitForFunction(({id,key})=>{const r=JSON.parse(localStorage.getItem('amg-user:'+encodeURIComponent(id)+':'+key)||'null');return r&&!r.dirty&&r.value.complete;},{id:config.accounts.A.id,key},{timeout:60000});
  const saved=await accountRecord(page,config.accounts.A),record=saved.records?.[key];
  ...
  stage='account-B-login';await login(page,config.accounts.B);
  const other=await accountRecord(page,config.accounts.B);
  ...
  check(await restoredPage.evaluate(({id,key})=>localStorage.getItem('amg-user:'+encodeURIComponent(id)+':'+key)===null,{id:config.accounts.A.id,key}),'The restoration context begins without account A’s local progress cache');
  await login(restoredPage,config.accounts.A,'/course/lesson-01/');
  await restoredPage.waitForFunction(()=>document.querySelector('#completion-status').textContent.includes('complete'));
  const restored=await accountRecord(restoredPage,config.accounts.A);
  ...
  const result={reviewedAt:new Date().toISOString(),status:'passed',previewOrigin:origin,checks,runtimeErrors,actualRequiredQuestions:8,progressPostRequests:progressPosts,reportingStatuses:[...reportingStatuses],lessonDataSha256:digest(fixtureText),syntheticAccountTags:{A:digest(config.accounts.A.id).slice(0,12),B:digest(config.accounts.B.id).slice(0,12)},scope:'Actual password login/Identity, protected routes, server learning API, transcript completion, eight required answers, signout, second-user isolation and fresh-context restoration on the supplied Netlify preview. Reporting was explicitly disabled and each authenticated API observation required not_configured. No signup, recovery, invitation, real emails, Google Sheet validation, screenshots, traces, saved cookies or raw credentials in evidence.'};
  save(result);console.log(JSON.stringify({status:result.status,checks:checks.length,evidence:outputPath}));
 }catch(error){
  const result={reviewedAt:new Date().toISOString(),status:'failed',previewOrigin:origin,checks,runtimeErrors,failure:safeFailure(error),progressPostRequests:progressPosts,reportingStatuses:[...reportingStatuses],scope:'Synthetic actual-login acceptance; failed safely without logging exception text, credentials, request bodies or cookies.'};
