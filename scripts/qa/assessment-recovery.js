async page => {
 const root='http://127.0.0.1:4174/course/assessments/index.html',key='amg-life-assessments-v1',checks=[],errors=[];
 ...
 async function answerDue(correct){
  ...
  await page.locator('input[name="review-answer"]').nth(correct?q.answer:(q.answer+1)%4).check();await page.getByRole('button',{name:'Check review answer',exact:true}).click();return await page.evaluate(k=>JSON.parse(localStorage.getItem(k)),key);
 }
 ...
 await peer.locator('input[name="exam-answer"]').nth(1).check();await page.waitForFunction(()=>document.querySelector('input[name="exam-answer"]:checked')?.value==='1');
 ...
 await page.getByRole('button',{name:'Resume Timed Form A',exact:true}).click();await page.locator('input[name="exam-answer"]').nth(2).check();
