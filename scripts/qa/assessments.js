async page => {
 const checks=[],errors=[],root='http://127.0.0.1:4174/course/assessments/index.html',key='amg-life-assessments-v1';
 ...
 for(const id of ['D','A','B','C']){
  ...
  for(let i=0;i<form.questions.length;i++){
   ...
   if(!skip){
    ...
    if(id==='A'&&i===0){await page.locator('input[name="exam-answer"]').nth((answer+1)%4).check();await page.getByRole('button',{name:'Clear answer',exact:true}).click();check(await page.locator('input[name="exam-answer"]:checked').count()===0,'Clear answer works');}
    await page.locator('input[name="exam-answer"]').nth(answer).check();
 ...
 await page.locator('input[name="review-answer"]').nth(q.answer).check();await page.getByRole('button',{name:'Check review answer',exact:true}).click();
 ...
 await page.getByRole('button',{name:'Resume Timed Form A',exact:true}).click();check(await page.evaluate(()=>document.documentElement.scrollWidth<=innerWidth+1),'Question and navigator fit mobile');
